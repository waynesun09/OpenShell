...
| `supervisor.topology` | Sandbox pod topology. Leave as `combined` for the original full-enforcement path, set `sidecar` when the agent container should run non-root without added Linux capabilities, set `cni-sidecar` to move sidecar network rule setup into the OpenShell CNI plugin, or set `proxy-pod` to run network enforcement in a separate supervisor pod with NetworkPolicy isolation. Refer to [Topology](/kubernetes/topology). |
