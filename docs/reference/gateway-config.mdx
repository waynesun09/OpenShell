...
For a local single-player macOS gateway:

```toml
[openshell.gateway]
credential_drivers = ["macos-keychain"]

[openshell.credential_drivers.macos-keychain]
service = "com.nvidia.openshell.provider-credentials"
```
...
For `macos-keychain`, OpenShell stores provider credentials as generic password items in the default macOS Keychain. `service` namespaces those items and defaults to `com.nvidia.openshell.provider-credentials`. `keychain_path` is optional and must be absolute; use it only for an explicitly managed alternate keychain, such as local validation with a temporary keychain.
